...
resource "aws_vpc" "main" {
  ...
  tags = merge(local.common_tags, {
    Name                                        = "${var.environment}-absencesbo-vpc"
...
resource "aws_internet_gateway" "main" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.environment}-absencesbo-igw"
  })
}
...
# Allow the flow logs service (via this role) to write to the log group
resource "aws_iam_role_policy" "vpc_flow_logs_write_cw" {
  name = "${var.environment}-${var.project_name}-vpc-flow-logs-write-cw"
...
resource "aws_flow_log" "main" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy.vpc_flow_logs_write_cw
  ]
}
